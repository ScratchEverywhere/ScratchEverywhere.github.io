...
6. On Windows, make sure you select the correct installer for your CPU:
...
8. Once installed, **launch Docker Desktop** to make sure it’s running.
...
-	[GIMP](https://gimp.org) is great for these kinds of things, but you can use any image editing software you want.
- 	For the Vita's images, you will neet to get [`pngquant`](https://pngquant.org/) and run:
	```
 	cd /path/to/gfx/vita/
	pngquant --posterize 4 path/to/your_image.png same/path/output.png
	```
	in the command prompt (Win + R then type "cmd" and press enter on Windows, Find "Terminal" in the home menu on everything else),
	then rename output.png to the original image's name. When asked to overwrite it, click "Yes".

...
| `ERROR: permission denied while trying to connect to the docker API at unix:///var/run/docker.sock`                                          | Your user account does not have the proper permissions to run Docker commands.                     | Prefix the build command with `sudo`: `sudo docker build -f docker/Dockerfile.{platform} --target exporter -o . .` |
